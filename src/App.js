...
function App() {
  ...
  const handleSquareClick = (square) => {
    if (legalMoves.includes(square)) {
      ...
      if (moveResult) {
        ...
        setMoves(prevMoves => [...prevMoves.slice(0, currentMoveIndex + 1), moveResult]);
  ...
  const handleMouseDown = (e, square) => {
    if (e.button === 2) {
      ...
      setOverlay(prevOverlay => ({ ...prevOverlay, [square]: !prevOverlay[square] }));
      setLegalMoves([]);
      setPotentialCaptures([]);
      setSourceSquare(null);
    }
      ...
      if (x >= 0 && x <= boardRect.width && y >= 0 && y <= boardRect.height) {
        ...
        if (currentLine) {
          ...
          if (currentLine.start !== endSquare) {
            setOverlay(prevOverlay => ({ ...prevOverlay, [currentLine.start]: false }));
          }
        }
      }
    }
  ...
  const renderSquare = (row, col) => {
    ...
    return (
      ...
      >
        {renderPiece(square)}
        <div className={`label ${labelColor}`}>{col}{row}</div>
      </div>
    );
  };
  ...
  const MovesTable = ({ moves, currentMoveIndex }) => {
    return (
        ...
        <table>
          ...
          <tbody>
            {moves.reduce((rows, move, index) => {
              if (index % 2 === 0) {
                rows.push(
                  <tr key={index / 2} className={index <= currentMoveIndex ? 'current-move' : ''}>
                    <td>{index / 2 + 1}</td>
                    <td>{move.san}</td>
                    <td>{index + 1 < moves.length ? moves[index + 1].san : ''}</td>
                  </tr>
                );
              }
              return rows;
